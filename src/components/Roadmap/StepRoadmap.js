...
export function StepRoadmap({ counter, ImgStep }) {
  ...
  return (
    <div
      className={`roadmap_step md:flex ${
        counter !== 2 ? "flex-row-reverse" : ""
      } justify-between ${
        !isDark ? "text-secondaryTxt" : "text-secondaryTxtDark"
      }`}
    >
      <div className="roadmap_img md:flex-grow md:w-8/12">{ImgStep}</div>
      <div className="roadmap_text">
        <div
          className="r_counter font-bold"
          style={{ backgroundColor: isDark && "#1D272D" }}
        >
          {counter}
        </div>
