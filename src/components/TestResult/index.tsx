    ...
    <div className="pt-[60px] flex flex-col items-center w-full">
      <div className="w-full max-w-5xl flex flex-col items-center text-center px-3">
        ...
        <div className="text-center w-full px-3">
          <div className="grid grid-cols-1 sm:grid-cols-2 w-full mt-16">
            <div className={`text-center border-r w-full px-3 max-h-[340px] ${isDark ? "border-gray/5" : "border-gray"}`}>
              <h3 className="pb-5 text-2xl font-bold">{t.by_land_type}</h3>
              {data && <PieChart labels={[t.lbl_built, t.lbl_forest, t.lbl_crop, t.lbl_graz, t.lbl_fish, t.lbl_energy]} chartData={[data.data.built, data.data.forest, data.data.crop, data.data.graz, data.data.fish, data.data.energy]} />}
            </div>

            <div className="text-center w-full px-3 max-h-[340px]">
              <h3 className="pb-5 text-2xl font-bold">{t.by_consumption}</h3>
              {data && <BarChart labels={[t.lbl_food, t.lbl_housing, t.lbl_transport, t.lbl_goods, t.lbl_services]} chartData={[data.data.food, data.data.housing, data.data.transport, data.data.goods, data.data.services]} />}
            </div>
          </div>
        </div>
        {/* ecopoints */}
        <EcoPoints total={data && data.data.total} />
      </div>
      {/* SignupCta */}
      <SignupCta />
    </div>
